...
if __name__ == "__main__":
    ...
    controlnet = ControlNetModel.from_pretrained("./output_models/controlnet").to(device)
    ...
    ipfms = IPFMs.from_pretrained("./output_models/ipfms").to(device)

    vae.requires_grad_(False)
    llm_model.requires_grad_(False)
    unet.requires_grad_(False)
    controlnet.requires_grad_(False)
    ipfms.requires_grad_(False)
    ...
    for image_idx, image_name in enumerate(tqdm(image_names, desc="Processing images")):
        ...
        with torch.no_grad():
            ...
            prompt_embeds = ipfms(text_embedding, timesteps)
